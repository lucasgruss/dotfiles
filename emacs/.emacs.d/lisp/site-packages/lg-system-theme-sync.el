...
(defcustom system-theme-sync-default-light-plist '(:background nil
						   :gtk-theme nil
						   :icon-theme nil)
  "Generic settings to apply to light themes.")

(defcustom system-theme-sync-default-dark-plist '(:background nil
						  :gtk-theme nil
						  :icon-theme nil)
...
(defcustom system-theme-sync-gtk-cmd "xfconf-query -c xsettings -p /Net/ThemeName -s %s"
  "The command used to change gtk-theme")

(defcustom system-theme-sync-icon-cmd "xfconf-query -c xsettings -p /Net/IconThemeName -s %s"  
  "The command used to change icon-theme.")

(defcustom system-theme-sync-background-cmd "xfconf-query -c xfce4-desktop -p  /backdrop/screen0/monitoreDP-1/workspace0/last-image -s %s"
  "The command used to change background.")
...
applied to gnome-settings or xfce-conf."
  (let* ((settings (or (plist-get system-theme-sync-per-theme-settings-plist theme)
    ...
    (when gtk-theme
      ...
      (efs/run-in-background (format system-theme-sync-gtk-cmd gtk-theme)))
    (when icon-theme
      ;(shell-command (format "sed -i 's/IconThemeName.*/IconThemeName \"Papirus-Light\"/g' ~/.xsettingsd" icon-theme))
      (efs/run-in-background (format system-theme-sync-icon-cmd icon-theme)))
    (when background
      (efs/run-in-background (format system-theme-sync-background-cmd  background))
      (efs/run-in-background (format system-theme-sync-background-cmd background))))
...
;;; lg-system-theme-sync ends here
